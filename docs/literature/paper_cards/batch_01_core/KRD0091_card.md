# Paper Card: EN003 Machine learning algorithm for estimating karst rocky desert
...
Use the full reader and Abstract/Introduction anchors to confirm exact objectives. Current grounded objective signal: EN003 Machine learning algorithm for estimating karst rocky desert

## 3. Study Area

Study area anchors: p.1, p.2, p.4, p.7, p.8, p.9, p.10, p.11. Exact place names require manual confirmation from the PDF.
...
- Metrics: IoU, Kappa, OA, PA, Precision, UA, accuracy, overall accuracy, slope
...
Located limitation/future-work evidence pages: p.2, p.8, p.8. Manual confirmation needed.
