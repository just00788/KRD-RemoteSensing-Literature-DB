...
- Literature positioning: load `references/literature-map.md`.
...
## Verification / Caution Rules

- Treat Batch 03 materials as quick-reading support only, not full paper-card evidence.
- Use Batch 03 to improve caution rules, method taxonomy, and literature-gap awareness; do not use it to assert verified numerical results.
- Keep LSMM/SMA/MESMA, feature-space indices, object-oriented extraction, machine learning, and semantic segmentation separate unless a source explicitly connects them.
- DeepLabV3+ and Chinese bare-rock extraction literature can strengthen method motivation, but manuscript performance claims must come from the user's own experiments.
- Chinese metadata, OCR-corrupted titles, and PDF-only records require manual verification before citation-level use.
